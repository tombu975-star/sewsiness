...
export function ShopBottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-30 border-t border-border bg-surface px-2 pb-[max(0.5rem,env(safe-area-inset-bottom))] pt-2">
            ...
            >
              <span
                className={`relative flex h-6 w-6 items-center justify-center ${
                  active ? "text-indigo" : "text-ink-faint"
                }`}
              >
                <Icon />
                {tab.href === "/shop/cart" && count > 0 && (
                  <span className="absolute -right-2 -top-2 flex h-4 min-w-[16px] items-center justify-center rounded-full bg-burgundy px-1 text-[9px] font-bold text-white">
                    {count}
                  </span>
                )}
              </span>
              <span className={active ? "text-indigo" : "text-ink-faint"}>{tab.label}</span>
            </Link>
          );
        })}
      </div>
