...
export default async function BusinessHealthPage() {
  await requirePageFeature(["owner", "manager"], "business_health");
  ...
  const overdueOrders = (orders ?? []).filter((o: any) => o.due_date && new Date(o.due_date) < new Date() && !["Delivered", "Cancelled"].includes(o.status)).length;
