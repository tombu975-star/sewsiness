    ...
    <main className="min-h-screen flex items-center justify-center px-4 py-10">
      <section className="card w-full max-w-xl p-8 text-center">
        ...
        {!certificate ? <><h1 className="font-display text-2xl font-semibold text-ink">Certificate not found</h1><p className="text-sm text-ink-muted mt-2">This verification code is invalid or the certificate is not available.</p></> : <>
          ...
          <div className={`inline-flex mt-4 rounded-full px-3 py-1 text-xs font-bold ${certificate.revoked_at ? "bg-danger-soft text-danger" : "bg-success-soft text-success"}`}>
            {certificate.revoked_at ? "Revoked certificate" : "Verified certificate"}
          </div>
          ...
          <div className="mt-6 space-y-3 text-left border-t border-border pt-5">
            ...
            <div className="flex justify-between gap-4"><span className="text-ink-muted">Issued</span><strong className="text-ink">{new Date(certificate.issued_at).toLocaleDateString()}</strong></div>
            <div className="flex justify-between gap-4"><span className="text-ink-muted">Training provider</span><strong className="text-ink">{certificate.business_name}</strong></div>
          </div>
